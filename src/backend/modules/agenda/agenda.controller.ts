...
export class AgendaController {
  ...
  async getMiAgenda(request: Request) {
    try {
      ...
      const citas = await this.service.listarMiAgenda(barberoId, fecha || undefined, fechaInicio || undefined, fechaFin || undefined);
  ...
  async postConfigurarHorario(request: Request) {
    try {
      ...
      return NextResponse.json({ success: true, message: "Horario guardado correctamente", data: resultado }, { status: 201 });
    } catch (error: any) {
  ...
  async deleteHorario(request: Request) {
    try {
      ...
      if (!cedula || !dia) return NextResponse.json({ success: false, message: "Faltan parámetros" }, { status: 400 });
